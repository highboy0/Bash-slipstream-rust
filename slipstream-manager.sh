...
download_binaries() {
    ...
    if whiptail --title "دانلود باینری" --yesno "آخرین نسخه Slipstream را دانلود کنیم؟" $HEIGHT $WIDTH; then
        whiptail --title "در حال دانلود" --infobox "در حال دریافت اطلاعات آخرین نسخه..." $HEIGHT $WIDTH
        
        # 1. استفاده از API گیت‌هاب برای دریافت اطلاعات آخرین ریلیز
        RELEASE_DATA=$(curl -s https://api.github.com/repos/highboy0/Bash-slipstream-rust/releases/latest)
        
        # 2. استخراج لینک دانلود مستقیم (بر اساس نام فایلی که در ریلیز گذاشتید)
        # نکته: در لینک شما نام فایل sliprstream.tar.gz است (یک r اضافه دارد)
        ASSET_URL=$(echo "$RELEASE_DATA" | jq -r '.assets[] | select(.name == "sliprstream.tar.gz") | .browser_download_url')

        if [[ -z "$ASSET_URL" || "$ASSET_URL" == "null" ]]; then
            whiptail --title "خطا" --msgbox "فایل sliprstream.tar.gz در ریلیز یافت نشد.\nلطفاً آدرس ریلیز را بررسی کنید." $HEIGHT $WIDTH
            exit 1
        fi
        ...
        # استخراج فایل
        # اگر در فایل tar.gz شما پوشه وجود ندارد، --strip-components=0 درست است
        tar -xzf slipstream.tar.gz
        
        rm slipstream.tar.gz
        
        # بررسی وجود فایل‌ها و تغییر سطح دسترسی
        if [[ -f "slipstream-server" ]]; then
            ...
            whiptail --title "خطا" --msgbox "فایل‌های اجرایی در آرشیو یافت نشدند." $HEIGHT $WIDTH
        fi
    fi
}
